...
class CronEmail extends EmailMessage {
    ...
    public function __construct($sendAccName, $receivers = []) {
        ...
        if ($activeJob !== null) {
            ...
            $file = new File(ROOT_DIR.DS.APP_DIR.DS.'sto'.DS.'logs'.DS.'cron'.DS.$activeJob->getJobName().'-ExecLog-'.date('Y-m-d H-i-s').'.log');
    ...
    private function _createJobInfoTable($job) {
        ...
        $jobTable->addChild($this->_createTableRow('Application Directory:', ROOT_DIR.DS.APP_DIR));
